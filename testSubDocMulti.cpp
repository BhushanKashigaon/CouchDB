...
// cluster_run mode
#define DEFAULT_CONNSTR "couchbase://10.10.4.42/Bhushan_bucket"
...
int main(int argc, char **argv) {
    ...
    for (int ii = 0; ii < 1; ii++) {
        ...
        printf("\n&path : %p  &val : %p ",&path,&val);
        char pbuf[1024], vbuf[1024];

        strncpy(pbuf, "plmn",sizeof("plmn"));
        strncpy(vbuf, "{\"mcc\":\"555\",\"mnc\":\"11\"}",sizeof("{\"mcc\":\"555\",\"mnc\":\"11\"}"));
        path = pbuf;
        val = vbuf;
        lcb_SDSPEC spec = { 0 };
